...
# Reference existing GitHub PAT secret — only needed when creating the connection.
...
resource "google_cloudbuildv2_repository" "repo_staging" {
  ...
  location          = var.region
  name              = var.repository_name
  parent_connection = "projects/${var.staging_project_id}/locations/${var.region}/connections/${var.staging_connection_name}"
  remote_uri        = "https://github.com/${var.repository_owner}/${var.repository_name}.git"
  depends_on = [
    resource.google_project_service.cicd_services,
    resource.google_project_service.deploy_project_services,
