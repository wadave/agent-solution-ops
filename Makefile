...
playground:
	...
	@echo "|                                                                             |"
	@echo "| 🔍 IMPORTANT: Select the 'src/adk_agent' folder to interact with your agent.       |"
	@echo "==============================================================================="
...
deploy:
	...
	(uv export --no-hashes --no-header --no-dev --no-emit-project --no-annotate > src/adk_agent/app_utils/.requirements.txt 2>/dev/null || \
	uv export --no-hashes --no-header --no-dev --no-emit-project > src/adk_agent/app_utils/.requirements.txt) && \
	uv run deployment/deploy_agents.py \
		--requirements-file=src/adk_agent/app_utils/.requirements.txt
...
test:
	uv sync --dev
	PYTHONPATH=src uv run pytest tests/unit && PYTHONPATH=src uv run pytest tests/integration
...
eval:
	...
	uv run adk eval ./src/adk_agent $${EVALSET:-tests/eval/evalsets/basic.evalset.json} \
